...
function assertsOnExistence(node, identifierName, shouldBeDefined) {
    ...
    // Is the expression represented by `node` performing
    // a `typeof identifierName` check against a literal?
    const isTypeofCheckAgainstLiteral =
    ...
    return isTypeofCheckAgainstLiteral &&
        (shouldBeDefined
...
function isGuardedUsage(node) {
    ...
    while (node.parent && expressionContainers.indexOf(node.parent.type) === -1) {
        ...
        if (node.type === "ConditionalExpression") {
            if ((assertsOnExistence(node.test, origIdentifierName, true) && node.consequent === prevNode) ||
                    (assertsOnExistence(node.test, origIdentifierName, false) && node.alternate === prevNode)) {
                return true;
            }
        }
    }
